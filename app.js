 ...
 * Provide helper functions to manage the state.
 */
class AppState {
  constructor() {
    this.searchResult = [];
  ...
  updateUI() {
    ...
    if (this.searchKeyword && this.searchResult.length === 0) {
      // There is no search result.
      resultDiv.innerHTML = `No videos found with the keyword ${this.searchKeyword}`;
    } else {
      // There are matching videos.
      resultDiv.innerHTML = this.searchResult.map(
        (result) => generateRowHTML(result)
      ).join('\n');
    }
  }

  /**
   * Filter videos by `keyword` and put the result in `searchResult`.
  ...
  search(keyword) {
    ...
    this.searchResult = videosList.filter(
      (video) => video.description.toUpperCase().includes(keyword.toUpperCase())
    );
    this.updateUI();
  }
...
function setup() {
  ...
  document.querySelector('#btn-show-all').addEventListener('click', handleShowAll);
}
